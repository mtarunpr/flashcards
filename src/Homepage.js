  ...
  getDeckHtml = deckId => {
    return (
      <Link key={deckId} className="flex-item" to={'/viewer/' + deckId}>
        <div className="name">{this.state.decks[deckId]['name']}</div>
        <br />
    ...
    return (
      <div>
        ...
        <div className="flex-container">
          {myDecks.length
            ? myDecks
            : 'You have not created any decks. Create one now!'}
        </div>

        <Link
          style={{ marginLeft: '20px' }}
          className="material-icons new-btn"
          to="/editor"
        >
          add_circle
        </Link>
        ...
        <div className="flex-container">
          {publicDecks.length
            ? publicDecks
            : 'There are no public decks created by others.'}
        </div>
      </div>
    );
  }
